...
- `mcp_server.py`: two new agent tools — `run_separability_test` and
  `run_anomaly_detection` (10 tools total). *(The entire MCP/AI layer was
  removed post-v0.1 — see "Post-v0.1" below.)*
...
Changes after the build-order queue completed (current suite: **101
passed**):

- **AI/MCP layer removed** (commit `2219984`): deleted `agent_workflow.py`,
  `agent_workflow_groq.py`, `demo_agent.py`, `mcp_server.py`, and
  `AI_INTEGRATION.md`; dropped the `[ai]`/`[ai-groq]` extras and their
  transitive deps from `uv.lock`. The core library, `demo.py`, and tests
  are unaffected. The `mcp_server.py` mentions in the "Follow-up" section
  above are therefore historical.
- **UI-independent figures** (commit `213c73a`): all chart logic moved to
  `results/figures.py` (`figures_for_result` / `figures_for_run` /
  `headline_metrics`), shared by the dashboard (now a thin renderer), the
  HTML report, and `Run.figures()`. Analyses emit flattened top-level
  frames so iconic plots survive a `ResultStore` round trip. New tests:
  `tests/test_figures.py`, `tests/test_cluster_heatmap.py`.
- **`make_constant_counter`** built-in feature added (commit `4e012c3`)
  with unit tests.
