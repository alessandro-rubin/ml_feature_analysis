...
def test_runner_skips_supervised_without_labels():
    ...
    analyses = [
        ...
        AnomalyDetection(run_lof=False, run_mahalanobis=False,
                         iforest_params={"n_estimators": 50, "n_jobs": 1}),
    ]
...
def test_separable_classes_get_small_p():
    ctx = AnalysisContext(df=_labeled_df(separable=True), cfg=Config(),
                          target_col="class")
    out = SeparabilityTest(n_permutations=50,
                           rf_params={"n_estimators": 50, "n_jobs": 1}).run(ctx)
...
def test_random_labels_are_not_separable():
    ctx = AnalysisContext(df=_labeled_df(separable=False), cfg=Config(),
                          target_col="class")
    out = SeparabilityTest(n_permutations=50,
                           rf_params={"n_estimators": 50, "n_jobs": 1}).run(ctx)
    s = out["summary"].iloc[0]
